      ...
      <DisplayPanel className="pointer-events-auto rounded-[2rem] bg-white/68 p-3 shadow-[0_20px_50px_rgba(15,23,42,0.10)] backdrop-blur-2xl">
        <DisplayPanelContent className="p-0">
          ...
          <div className="relative space-y-2 pl-4">
            ...
            <div
              className="absolute left-[1.3rem] top-3 w-px bg-gradient-to-b from-sky-300 via-cyan-400 to-violet-300 transition-all duration-500"
              style={{ height: `calc(${progressHeight} - 0.75rem)` }}
            />
            {SCENES.map((scene, index) => {
              ...
              return (
                <a
                  ...
                  href={scene.href}
                  className={cn(
                    'group relative flex items-center gap-3 rounded-full px-3 py-2 text-[11px] font-bold transition-all',
                    active
                      ? 'bg-sky-500 text-white shadow-[0_10px_24px_rgba(14,165,233,0.20)]'
                      : reached
                ...
                >
                  <span
                    className={cn(
                      'absolute left-[0.18rem] top-1/2 h-2.5 w-2.5 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white transition-all',
                      active
                        ? 'bg-white shadow-[0_0_0_5px_rgba(14,165,233,0.22)]'
                        : reached
                          ? 'bg-sky-400'
                  ...
                  <Badge
                    variant="outline"
                    className={cn(
                      'inline-flex h-6 w-6 items-center justify-center rounded-full px-0 text-[10px]',
                      active
