...
export function MarketToolIcon({ tool, size = 'md' }: MarketToolIconProps) {
  ...
  const boxClassName = size === 'sm' ? 'h-10 w-10 rounded-2xl text-lg' : 'h-12 w-12 rounded-2xl text-xl'

  return (
    <div className={cn('flex shrink-0 items-center justify-center overflow-hidden border border-border/60 bg-white shadow-sm', boxClassName)}>
