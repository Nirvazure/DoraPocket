...
type ResolvedPocketTool = {
  entry: ReturnType<typeof useStore.getState>['pocketInventory'][number]
  tool: NonNullable<ReturnType<typeof getToolById>>
}

function resolvePocketTools(items: ReturnType<typeof useStore.getState>['pocketInventory']) {
  return items
    ...
    <article key={entry.toolId} className="flex min-h-56 flex-col rounded-3xl border border-border/60 bg-slate-50 p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-wrap justify-end gap-1.5">
          {entry.pinned ? (
            <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold text-amber-800">已置顶</span>
          ) : null}
          {entry.purchased ? (
            <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[10px] font-bold text-emerald-800">已购买</span>
          ) : null}
        </div>
      </div>
      ...
      <div className="mt-auto pt-4">
        <div className="mt-3 flex flex-wrap gap-2">
          <Button type="button" variant="outline" className="h-9 rounded-full bg-white px-3 text-xs" onClick={() => togglePinTool(entry.toolId)}>
            <Pin className="mr-1.5 h-3.5 w-3.5" />
            {entry.pinned ? '取消置顶' : '置顶'}
          </Button>
          <Button type="button" variant="outline" className="h-9 rounded-full bg-white px-3 text-xs" onClick={() => togglePurchasedTool(entry.toolId)}>
            <CheckCircle2 className="mr-1.5 h-3.5 w-3.5" />
            {entry.purchased ? '取消已购' : '标记已购'}
          </Button>
          <Button type="button" variant="outline" className="h-9 rounded-full bg-white px-3 text-xs" onClick={() => removeToolFromPocket(entry.toolId)}>
            移出收藏
          </Button>
          {tool.url ? (
    ...
    <PageShell
      header={
        <UnifiedTopBar
          ...
          subtitle="把你从市场挑中的工具收进来，留成自己的收藏夹。"
          rightSlot={
        ...
        <>
          {pinnedTools.length > 0 ? (
            <section className="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-xl shadow-slate-900/8 backdrop-blur-xl">
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {pinnedTools.map(renderPocketCard)}
              </div>
            </section>
          ) : null}
          ...
          <section className="rounded-[2rem] border border-white/80 bg-white/90 p-5 shadow-xl shadow-slate-900/8 backdrop-blur-xl">
            ...
            {allCollectedTools.length === 0 ? (
              ...
              <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {allCollectedTools.map(renderPocketCard)}
              </div>
            )}
          </section>
        </>
            ...
            {archivedTools.map(({ entry, tool }) => (
              <article key={entry.toolId} className="flex min-h-48 flex-col rounded-3xl border border-border/60 bg-slate-50 p-4">
                ...
                <div className="mt-auto pt-4 flex flex-wrap gap-2">
                  <Button type="button" variant="outline" className="h-9 rounded-full bg-white px-3 text-xs" onClick={() => toggleArchiveTool(entry.toolId)}>
                    取消归档
                  </Button>
                  <Button type="button" variant="outline" className="h-9 rounded-full bg-white px-3 text-xs" onClick={() => removeToolFromPocket(entry.toolId)}>
                    删除
