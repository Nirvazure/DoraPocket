...
export function CompactDecisionPanel({
  ...
  const reason = payload?.selectionReason ?? 'DoraPocket 会在分析任务后给出这次最值得先用的帮助。'
    ...
    return (
      <DisplayPanel className="rounded-[2rem] border-dashed bg-white p-6 text-center shadow-sm">
        <DisplayPanelHeader className="p-0">
          ...
          <DisplayPanelTitle className="mt-2 text-2xl">
            等待 DoraPocket 给出主推荐
          </DisplayPanelTitle>
        </DisplayPanelHeader>
    ...
    <DisplayPanel className="overflow-hidden rounded-[2rem] border-primary/15 bg-white shadow-xl shadow-primary/5">
      <div className="grid gap-0 xl:grid-cols-[1.25fr_0.75fr]">
        <DisplayPanel className="rounded-none border-0 bg-slate-950 p-5 text-white shadow-none sm:p-6">
          ...
          <DisplayPanelTitle className="mt-4 text-4xl leading-tight text-white">
            {title}
          </DisplayPanelTitle>
          ...
          {payload?.selectionSignals.length ? (
            <div className="mt-4 flex flex-wrap gap-2">
              {payload.selectionSignals.slice(0, 3).map((signal) => (
                <Badge
                  key={signal}
                  className="border-white/12 bg-white/10 px-3 py-1 text-[11px] font-semibold text-white/82"
                >
                  {signal}
                </Badge>
              ))}
          ...
          {leaderExternalUrl ? (
            <div className="mt-5 flex flex-wrap gap-2">
              <Button
                type="button"
                className="h-10 rounded-full bg-white px-4 text-xs font-bold text-slate-950 hover:bg-white/90"
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <Button
                type="button"
                variant="outline"
                className="h-10 rounded-full border-white/20 bg-white/10 px-4 text-xs font-bold text-white hover:bg-white/16"
