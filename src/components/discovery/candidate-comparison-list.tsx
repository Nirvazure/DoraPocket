  ...
  return (
    <DisplayPanel className="rounded-3xl border-border/55 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        ...
        <Badge
          variant="outline"
          className="bg-slate-100 px-3 py-1 text-[11px] font-semibold text-muted-foreground"
        >
          最多 3 个
        </Badge>
      </div>
          ...
          {candidates.map((candidate, index) => {
            ...
            return (
              <DisplayPanel
                ...
                className={
                  isLeader
                    ? 'rounded-3xl border-primary/25 bg-primary/[0.06] p-4 shadow-none'
                    : 'rounded-3xl border-border/55 bg-slate-50 p-4 shadow-none'
                }
              >
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {candidate.candidateType === 'external_suggestion' && candidate.url ? (
                    ...
                    <Button
                      ...
                      size="sm"
                      className="h-8 rounded-full px-3 text-[11px]"
                      onClick={() => onLaunchCandidate(candidate.toolId!)}
                    >
                      <ExternalLink className="mr-1.5 h-3.5 w-3.5" />
                      打开
                    </Button>
                  ...
                  {candidate.toolId ? (
                    <Button
                      ...
                      variant="outline"
                      className="h-8 rounded-full px-3 text-[11px]"
                      onClick={() => onOpenCandidate(candidate.toolId!)}
                    >
                      比较依据
                    </Button>
                  ) : null}
