...
export function CandidateComparisonList({ payload, onOpenCandidate, onLaunchCandidate }: CandidateComparisonListProps) {
  ...
  return (
    <section className="rounded-3xl border border-border/55 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">候选比较</p>
          <h3 className="mt-1 text-lg font-black text-foreground">为什么是它，而不是另外两个</h3>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-[11px] font-semibold text-muted-foreground">最多 3 个</span>
      </div>
      {candidates.length === 0 ? (
        <div className="mt-4 rounded-2xl border border-dashed border-border bg-slate-50 px-4 py-6 text-center text-sm text-muted-foreground">
        ...
        <div className="mt-4 grid gap-3">
          {candidates.map((candidate, index) => {
            ...
            return (
              <article key={candidate.toolId} className={isLeader ? 'rounded-3xl border border-primary/25 bg-primary/[0.06] p-4' : 'rounded-3xl border border-border/55 bg-slate-50 p-4'}>
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-2 rounded-2xl border border-white/80 bg-white/80 px-3 py-2 text-[11px] leading-relaxed text-foreground/75">
                      {isLeader ? '首选原因：这一项是当前任务最稳的起点。' : `暂不首选：当前轮次里，TOP 1 的综合信号更强。`}
                    </p>
                  </div>
                  <span className="rounded-full border border-border/55 bg-white px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">
