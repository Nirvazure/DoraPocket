...
export function DecisionHero({
  ...
  const reason =
    ...
    '输入任务后，DoraPocket 会先比较候选，再给出当前最值得用的工具。'
  ...
  return (
    <DisplayPanel className="relative overflow-hidden rounded-[2rem] border-primary/20 bg-slate-950 px-5 py-5 text-white shadow-2xl shadow-slate-950/20 sm:px-6 sm:py-6">
      ...
      <div className="relative z-10">
        <Badge variant="dark" className="w-fit border-white/15 bg-white/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.2em] text-white/80">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_auto] xl:items-end">
          <DisplayPanelHeader className="p-0">
            ...
            <DisplayPanelTitle className="mt-2 text-4xl leading-none text-white sm:text-5xl">{title}</DisplayPanelTitle>
          ...
          <DisplayPanelContent className="flex flex-wrap gap-2 p-0 xl:w-52 xl:flex-col">
            {leadingToolId ? (
              ...
              <Badge variant="dark" className="border-white/15 bg-white/10 px-4 py-2 text-xs font-semibold text-white/76">
                等待任务输入
              </Badge>
            )}
          </DisplayPanelContent>
