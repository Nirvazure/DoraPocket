...
export function UnifiedTopBar({
  ...
  const content = (
    <div
      className="grid w-full grid-cols-[auto_1fr_auto] items-center gap-3 rounded-[1.55rem] border border-white px-3 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_8px_24px_rgba(15,23,42,0.05)] bg-white"
    >
      ...
      <div className="min-w-0 text-center sm:text-left">
        <Link href="/" className="group inline-flex min-w-0 flex-col items-center sm:items-start">
          <div className="flex min-w-0 flex-wrap items-center justify-center gap-2 sm:justify-start">
            <p className="truncate font-sans text-sm font-black tracking-tight text-foreground transition-colors group-hover:text-primary sm:text-base">{title}</p>
          </div>
          {subtitle ? <p className="mt-0.5 truncate text-[11px] text-muted-foreground transition-colors group-hover:text-foreground/80 sm:text-xs">{subtitle}</p> : null}
        </Link>
      </div>
  ...
  return (
    <header className={cn('shrink-0 w-full', className)}>
      <div
        className="relative w-full rounded-[2rem] border border-white bg-white p-1.5 shadow-[0_10px_28px_rgba(15,23,42,0.06)] backdrop-blur-2xl"
      >
        <div className="pointer-events-none absolute inset-x-8 top-1.5 h-px bg-gradient-to-r from-transparent via-white to-transparent" aria-hidden />
        {content}
      </div>
    </header>
  )
