...
import { IntroContrastSection } from '@/components/intro/intro-contrast-section'
...
const SCENE_STYLES = {
  ...
  contrast: {
    rootBackground:
      'bg-[radial-gradient(circle_at_15%_18%,rgba(192,132,252,0.16),transparent_22%),radial-gradient(circle_at_78%_22%,rgba(125,211,252,0.18),transparent_26%),linear-gradient(180deg,#f7f9ff_0%,#f3f4ff_34%,#eef4ff_100%)]',
    leftGlow: 'bg-fuchsia-300/18',
    rightGlow: 'bg-violet-400/24',
    bottomGlow: 'bg-sky-200/20',
    ambientTint: 'from-white/0 via-violet-100/55 to-white/0',
    meshTone: 'from-violet-100/60 via-white/0 to-sky-100/28',
  },
  market: {
    rootBackground:
...
function IntroSceneDivider({ from, to }: { from: string; to: string }) {
  return (
    <div aria-hidden className="relative flex h-24 items-center justify-center overflow-hidden sm:h-28">
...
export function IntroPage() {
  ...
  const contrastRef = useRef<HTMLElement | null>(null)
  ...
  return (
    <div ref={rootRef} data-active-scene={activeScene} className={cn('min-h-screen text-slate-900 transition-[background] duration-700 [--intro-grid-opacity-value:0.4] [--intro-stage-scale:1] [--intro-stage-y:0px]', sceneStyle.rootBackground)}>
      <div aria-hidden className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className={cn('absolute left-[-8rem] top-24 h-72 w-72 rounded-full blur-3xl transition-all duration-700', sceneStyle.leftGlow)} />
        <div className={cn('absolute right-[-6rem] top-[22rem] h-80 w-80 rounded-full blur-3xl transition-all duration-700', sceneStyle.rightGlow)} />
        <div className={cn('absolute bottom-20 left-1/2 h-96 w-96 -translate-x-1/2 rounded-full blur-3xl transition-all duration-700', sceneStyle.bottomGlow)} />
        <div className={cn('absolute inset-x-0 top-[8vh] h-[38vh] bg-gradient-to-b blur-3xl transition-all duration-700', sceneStyle.ambientTint)} />
        <div className={cn('absolute inset-x-0 top-[16vh] h-[44vh] bg-gradient-to-r blur-3xl transition-all duration-700', sceneStyle.meshTone)} />
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0),rgba(255,255,255,0.5)_68%,rgba(255,255,255,0.78)_100%)] transition-all duration-700" style={{ opacity: 'var(--intro-grid-opacity-value)', transform: 'translate3d(0,var(--intro-stage-y),0) scale(var(--intro-stage-scale))' }} />
      </div>
      ...
      <main className="relative mx-auto flex max-w-[1440px] flex-col px-4 pb-24 pt-4 sm:px-6 lg:px-8">
        ...
        <IntroContrastSection sectionRef={contrastRef} />
        ...
        <IntroSceneDivider {...SCENE_DIVIDERS[5]} />
